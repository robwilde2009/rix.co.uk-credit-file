...
def ch_session():
    ...
    s.headers.update({
        ...
        "User-Agent": "rix-credit-api/2.2.1",
    })
    return s
...
def get_latest_accounts(company_number: str):
    ...
    for item in filings.get("items", []):
        ...
        pdf_url = metadata.get("links", {}).get("document") or f"{meta_url.rstrip('/')}/content"
...
def credit_assessment(company_number: str):
    try:
        ...
        pdf = doc_pdf(data["pdf_url"])
        extracted = extract_financials_from_pdf_bytes(pdf, company_number)
        scoring = score_financials(extracted, profile=profile, charges=charges)
